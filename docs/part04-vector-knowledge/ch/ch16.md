...
内部评测集要跟业务语言一起维护。新产品上线、组织架构调整、指标口径变化、合同模板更新后，原有 query 和 golden docs 可能不再覆盖真实问题。平台可以从失败会话、人工搜索日志、DataAgent schema linking 失败样例和客服转人工记录中持续抽样，把它们沉淀成 hard negatives 和回归样本。embedding 选型因此会进入检索质量运营，而不是停留在一年一次的模型采购讨论。
