...
This production example places semantic parsing, permission filtering, quality status, and lineage hints in one response. The goal is to make table and metric selection a governed platform decision rather than an Agent guess.
...
Such definitions allow DataAgent to reuse approved metric logic instead of dynamically stitching definitions for every query. The definition also gives reviewers a stable place to manage dimensions, timezone, source asset, quality requirements, and access policy.
...
The core logic is search, authorize, resolve the metric, check quality and lineage, then record audit. This process happens before SQL execution, which means unsafe or stale assets can be rejected before the Agent creates a confident answer.
