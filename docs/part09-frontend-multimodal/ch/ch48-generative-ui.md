...
Generative UI 的关键是让 Agent 在受控协议内选择、填充和编排平台登记过的组件，而不是让模型随手生成界面。模型可以建议“这里适合柱状图”“这里需要审批卡片”“这份报告可以形成 Artifact”，但最终渲染必须经过组件白名单、字段权限、数据引用和服务端策略。否则富交互会从用户体验优势变成新的攻击面和审计盲区。
