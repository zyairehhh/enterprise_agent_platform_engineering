...
```text
图片生成 Prompt（占位）：
为一本中文技术书生成一张横向架构图，主题是“Agent 运行轨迹采集与质量闭环”。画面从左到右分为三段：左侧是一次用户会话 Session，包含多个 Turn，并触发多个 Run；中间是运行时采集链路，包括 Context Package、Step、Model Call、Tool Call、Trace、Log、Metric、Artifact；右侧是 AgentOps 闭环，包括 Replay 回放、Root Cause 归因、Benchmark 样本沉淀、Prompt/Tool/Schema 修复、Regression 回归、Online Monitor 监控。风格为企业级技术架构图，白底，蓝灰主色，少量绿色强调闭环箭头，节点清晰，中文标签，避免卡通和过度装饰。
```
