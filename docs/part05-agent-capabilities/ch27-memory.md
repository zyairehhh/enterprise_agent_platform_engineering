...
Memory 与 Runtime、Planner、RAG 的读写关系如下（Run 主循环见 **图 P5-02**）：

| 路径 | 组件 | 动作 |
| --- | --- | --- |
| **写** | RunLoop → WorkingMemory | 追加 user / tool result；检查点 snapshot |
| **读** | Planner → WorkingMemory | `read window` 组装 prompt |
| **读** | Planner → MemoryStore / RAG | 按需 retrieve episodic / 文档 chunk |
| **出** | Working + Store + RAG → Gateway | 合并为 context bundle |
...
Working Memory 在 Run 主循环中的时机（与 **图 P5-02** 一致）：用户 POST run → append user → `next_step` 前 read window → Tool Call 后 append tool result → 检查点 snapshot WM。
...
晋升流程：`Run succeeded` → PII + 策略过滤 → 去重合并 → 写入 MemoryStore；Profile 更新走独立路径，**不自动** 从单次 Run 推断敏感字段。
...
Planner 组装 **context bundle** 时，三路来源合并后送 Gateway：

| 来源 | 读时机 | 典型内容 | 禁止 |
| --- | --- | --- | --- |
| Working Memory | 每步 `next_step` | 最近 N 轮 user/tool | 替代语义层（Ch.33） |
| MemoryStore | 按需 retrieve | Profile、episodic | 未经 promote 的 PII |
| RAG | 制度/文档问句 | cite `doc_id` | 当聊天历史索引 |
...
超长治理流水线：**估算 token** → 超阈值则 **LLM 摘要旧轮** → **滑窗保留** → 送 Gateway；压缩须保留最后一次成功 Tool 的关键字段、当前用户原问、进行中的 `tool_call_id` 链。
