...
**图 P5-01** Run 六态状态机 · `images/ch/p5-01-run-state-machine.png`

![Run 六态状态机](images/ch/p5-01-run-state-machine.png)



#### 迁移表

下表将 **图 P5-01** 中的每条边展开为可查询的迁移记录，便于对照 `state_machine.py`：
...
**图 P5-02** 端到端 Run 时序 · `images/ch/p5-02-run-sequence.png`

ReAct 模式下 Planner 的 `next_step` → invoke → 再 `next_step` 循环即本图所示；Plan-and-Execute 内部流程见 **图 P5-06**。

![端到端 Run 时序](images/ch/p5-02-run-sequence.png)
