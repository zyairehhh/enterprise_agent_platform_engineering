...
Host 内 Runtime、Registry 与 MCP Client 的关系见 **图 P5-03**（Registry 架构）与 **图 P5-05**（MCP 桥接闭环）；Client 可连接多个 MCP Server（Sidecar 或共享服务，见下文部署拓扑）。
...
三类能力在平台中的典型落点见上表；Planner 分别经 Tools / Resources / Prompts 取数后汇总回答——**不必**把三类能力塞进同一个 MCP Server，关键是 Host 侧统一发现与权限模型，再汇入 Registry 或 Memory（Prompts 进 Prompt 仓库）。
...
**图 P5-05** MCP → Registry 桥接闭环 · `images/ch/p5-05-mcp-registry-bridge.png`

![MCP → Registry 桥接闭环](images/ch/p5-05-mcp-registry-bridge.png)
