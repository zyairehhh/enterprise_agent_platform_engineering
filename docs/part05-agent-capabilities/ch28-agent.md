...
**图 P5-07** 单/多 Agent 决策树 · `images/ch/p5-07-multi-agent-decision-tree.png`

![单/多 Agent 决策树](images/ch/p5-07-multi-agent-decision-tree.png)
...
**图 P5-08** 山岚 Handoff 时序 · `images/ch/p5-08-handoff-sequence.png`

注意对外仍只有一个 `run_id`：

![Handoff 时序](images/ch/p5-08-handoff-sequence.png)
...
Handoff 在同一 `run_id` 内传递 payload（见 **图 P5-08**）：Workflow → Data → Report → 可选 return 至 Workflow；实现为 Registry `handoff@v1` Tool Call，切换 `active_agent_id`。
...
Router 数据流：**用户 input** → Workflow Router（读取 L1 Catalog + 路由规则 YAML）→ `handoff` Tool Call → RunLoop（与 **图 P5-08** 同一 Run）。
...
并行 Data Agent 查数后的典型流程：

1. D1、D2 并行返回 `metrics_v3`。  
2. Workflow Agent `diff(metrics)`。  
3. 一致 → 交 Reviewer 起草；不一致 → `escalate` 或 `waiting_human`（Ch.30）。
