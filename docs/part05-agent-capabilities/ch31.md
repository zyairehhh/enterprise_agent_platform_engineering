...
> **前置阅读**：[Ch.22 Agent Runtime](ch22-agent-runtime.md)–[Ch.30 Human-in-the-loop](ch30-human-in-the-loop.md)、[Ch.02 §2.2](../part01-overview/zh/ch02-agent.md)  
...
框架嵌入 Planner、应用走平台 Runtime 的分层见 **图 P5-01**–**P5-03** 与下表映射；**图 P5-12** 给出自研/采购/混合选型。
...
生产推荐：**LangGraph / CrewAI 仅作 Planner 实现**，Run 仍走平台 RunLoop（**图 P5-02**）与 Registry（**图 P5-03**）。
...
**图 P5-12** 框架/平台选型决策树 · `images/ch/p5-12-framework-selection-tree.png`

![框架/平台选型决策树](images/ch/p5-12-framework-selection-tree.png)
...
混合拓扑：Coze Bot / Dify Workflow → **API Gateway** → 企业 Runtime（**图 P5-02**）→ Registry → ERP / 语义层；LangGraph Planner 嵌入 L2，**不** 另起第二套 Run 六态。
