...
HITL 是 **图 P5-01** Run 六态中 `executing → waiting_human` 迁移的延伸：Console 经 SSE 展示 `approval_request`，审批人回调后 Runtime 恢复执行；引擎与业务检查点分工见 **图 P5-11**。
...
审批相关迁移见 **图 P5-01**（`executing → waiting_human → executing | failed`）。进入 `waiting_human` 时须持久化引擎与业务检查点（**图 P5-11**），并推送 SSE `approval_request`。
...
山岚后置审批步骤：Report Agent `handoff(report_md)` → Workflow 触发 `need_approval` → Console `approval_request` → 总监 `approved` → Runtime `invoke publish_report` → `succeeded`（状态迁移见 **图 P5-01**）。
...
Pod 重启恢复 `waiting_human` 的步骤：save 引擎检查点 → 重启 → load `run_id` → 幂等 sync `approval_request` 至 Console → 审批人 `approved` → Runtime `fire approved`（**不** 自动批准）。
...
RunLoop → **Task Queue** → Worker → 结果回写 RunLoop；同时更新 **引擎检查点**（恢复执行）与 **业务 milestone**（Console 可读）。二者分工见 **图 P5-11**。
...
**图 P5-11** 引擎检查点 vs 业务检查点 · `images/ch/p5-11-dual-checkpoint.png`

同一 `run_id` 下：引擎检查点负责 **恢复 RunLoop 执行**；业务检查点供 Console **展示进度与合规审计**。

![双检查点关系](images/ch/p5-11-dual-checkpoint.png)
