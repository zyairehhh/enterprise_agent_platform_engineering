...
多 Agent 协作最容易被误用成“多几个角色一起聊天”。生产系统里的角色代表责任边界，不是人格设定。一个 DataAgent 负责查询和分析，一个 Reviewer Agent 负责证据检查，一个 Workflow Agent 负责审批推进，它们之间必须通过 Handoff 契约交换结构化状态。若只是把上一个 Agent 的自然语言回答转给下一个 Agent，链路很快会丢失权限、证据和错误分类。
...
多 Agent 中的人类角色也要明确。某些交接需要业务负责人确认，某些冲突需要数据负责人仲裁，某些外发需要合规审批。人工是某些流程的正常参与者，不能被当作协作失败后的最后补丁。Runtime 应把人类节点和 Agent 节点放在同一条状态链里。
