...
**图 P5-09** L3 协议版图 · `images/ch/p5-09-l3-protocol-landscape.png`

读图时可记住：Runtime 只认 `invoke`，协议细节在适配层消化：

![L3 协议版图](images/ch/p5-09-l3-protocol-landscape.png)
...
MCP 三角色与 **图 P5-05**（Registry 桥接）、Ch.24 架构表一致：
...
山岚 Data Agent 经 A2A Client 委托外部舆情 Agent 的步骤：`delegate(task_spec)` → `CreateTask` / `SendMessage` → `TaskStatusUpdate` 循环 → `Task(artifacts)` → `structured_result`。与 Ch.28 内部 Handoff 的对照见下表。
...
1. 运维粘贴 Agent Card URL 至 L1 Catalog。  
2. L1 `fetch + validate` → Agent Card JSON。  
3. 映射为内部 **AgentSpec** → Ch.28 Router → A2A delegate 外部 Agent。
...
1. **入站**：ACP Message → 内部 Event Bus envelope（含 `run_id`, `tenant_id`）。  
2. **出站**：Run `action`/`result` → ACP 通知订阅 Agent（若 Policy 允许）。  
3. **禁止** ACP 绕过 Policy 直接触发 `invoke`（L3 位置见 **图 P5-09**）。
...
**图 P5-10** MCP + A2A 组合时序 · `images/ch/p5-10-mcp-a2a-combo-sequence.png`

![MCP + A2A 组合时序](images/ch/p5-10-mcp-a2a-combo-sequence.png)
