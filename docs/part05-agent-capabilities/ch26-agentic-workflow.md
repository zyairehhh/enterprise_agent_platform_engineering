...
Agentic Workflow 增强叠加在 Ch.25 Planner 与 Ch.22 Runtime 之上，**不改变 Run 六态（图 P5-01）**：Reflexion / Self-Refine / ToT 封装在 Planner 内部，Runtime 只看见本轮 `next_step` 耗时变长、`llm_call_count` 可能增加。
...
Reflexion 在 `next_step` 内部插入，不改变 Run 六态（与 **图 P5-02** 同一 Run 循环）：
...
ToT 搜索完成后，仅 **选中分支** 产生 Tool Call；未选中分支在 Planner 内存中剪枝（`tot_branching` × `tot_depth` 须受 Gateway 配额约束，Ch.38）。
