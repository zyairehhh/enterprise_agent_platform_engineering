...
模型部署的核心不是把权重跑起来，而是把模型服务变成稳定、可升级、可回滚的生产接口。单机运行 vLLM 或 SGLang 可以验证能力，进入企业平台后，还要处理镜像、权重、GPU 节点、就绪探针、流式连接、灰度切流和多环境晋升。业务 Agent 不应该感知这些细节。
...
企业模型发布要把权重、Runtime 镜像和关键参数当作不可变版本。一次发布不是改一行 YAML，而是产生可识别的 Revision，并经过探针、评测、灰度和回滚门禁。这样第45章的网关才能按服务名路由，而不是把底层推理进程暴露给业务应用。
